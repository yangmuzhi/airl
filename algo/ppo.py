...
class PPO(Base):
    def __init__(self, save_path, Policy_net, ob_space, act_space, gamma=0.95, clip_value=0.2, c_1=1, c_2=0.01):
        """
        :param Policy:
        :param Old_Policy:
        :param gamma:
    ...
    def train(self, obs, actions, gaes, rewards, v_preds_next):
        self.sess.run([self.train_op, self.loss], feed_dict={self.Policy.obs: obs,
    ...
    def act(self, obs, stochastic=True):
        if stochastic:
            return self.sess.run([self.Policy.act_stochastic, self.Policy.v_preds], feed_dict={self.Policy.obs: obs})
        else:
            return self.sess.run([self.Policy.act_deterministic, self.Policy.v_preds], feed_dict={self.Policy.obs: obs})

    def get_action_prob(self, obs):
        return self.sess.run(self.Policy.act_probs, feed_dict={self.Policy.obs: obs})
